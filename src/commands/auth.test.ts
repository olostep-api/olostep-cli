...
describe("auth set-token", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "auth-token-test-"));
    originalConfigDir = process.env.OLOSTEP_CLI_CONFIG_DIR;
    process.env.OLOSTEP_CLI_CONFIG_DIR = tmpDir;
  ...
  it("writes credentials.json with api_key field (token stored as api_key)", async () => {
    const program = makeProgram();
    await program.parseAsync(["node", "cli", "auth", "set-token", "tok-service-xyz"]);

    const credFile = path.join(tmpDir, "credentials.json");
    expect(fs.existsSync(credFile)).toBe(true);
    const data = JSON.parse(fs.readFileSync(credFile, "utf8"));
    expect(data.api_key).toBe("tok-service-xyz");
  });
  ...
  it("prints JSON output when --json flag is set", async () => {
    const program = makeProgram();
    await program.parseAsync(["node", "cli", "auth", "set-token", "tok-service-xyz", "--json"]);
  ...
  it("set-token and set-key produce identical credentials.json structure", async () => {
    const dir1 = fs.mkdtempSync(path.join(os.tmpdir(), "auth-cmp1-"));
    const dir2 = fs.mkdtempSync(path.join(os.tmpdir(), "auth-cmp2-"));

    try {
      process.env.OLOSTEP_CLI_CONFIG_DIR = dir1;
      const prog1 = makeProgram();
      await prog1.parseAsync(["node", "cli", "auth", "set-key", "shared-value"]);

      process.env.OLOSTEP_CLI_CONFIG_DIR = dir2;
      const prog2 = makeProgram();
      await prog2.parseAsync(["node", "cli", "auth", "set-token", "shared-value"]);

      const cred1 = JSON.parse(fs.readFileSync(path.join(dir1, "credentials.json"), "utf8"));
      const cred2 = JSON.parse(fs.readFileSync(path.join(dir2, "credentials.json"), "utf8"));
